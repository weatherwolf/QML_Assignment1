...
public class Assignment1Ex8  {
	...
	public static void main(String[] args) throws FileNotFoundException {
        ...
        try {
            ...
            solveAssignment1Ex8(x, y, vE, vA, sE, sA, cE, cA, T, alphas[j]);
            }
	...
	public static void solveAssignment1Ex8(int[] x, int[] y, double vE, double vA, double sE, double sA, 
                                           double cE_, double cA_, double T, double alpha) throws GRBException {
		...
		// Add the restiction that at least 1-alpha cities need to evacuate in T time.
        GRBLinExpr alphaExpr = new GRBLinExpr();
		for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) {
                double valueE = sE + 60*Functions.distE(x[i], y[i], x[j], y[j])/(vE);
				int boolE = Functions.binF(valueE, T);
				alphaExpr.addTerm(boolE, hE[i][j]);
				...
				alphaExpr.addTerm(boolA, hA[i][j]);
            }
        }
        double temp = (1-alpha)*n;
        model.addConstr(alphaExpr, GRB.GREATER_EQUAL, temp, "alpha Constraint");

        // Add constraint that the city can go to exactly one destination in 2T(evaucation center)
        for(int j = 0; j < n; j++) {
            GRBLinExpr destinationExpr = new GRBLinExpr();
            for(int i = 0; i < n; i++) {
                double valueE = sE + 60*Functions.distE(x[i], y[i], x[j], y[j])/(vE);
				int boolE = Functions.binF(valueE, 2*T);
				destinationExpr.addTerm(boolE, hE[i][j]);

				// calculating the time to travel to an advanced evacuation center
				double valueA = sA + 60*Functions.distA(x[i], y[i], x[j], y[j])/(0.5*vA);
				int boolA= Functions.binF(valueA, 2*T);
		...
		if (model.get(GRB.IntAttr.Status) == GRB.Status.OPTIMAL) {
			...
			System.out.println("Objective = " + model.get(GRB.DoubleAttr.ObjVal) + " alpha = " + alpha);
